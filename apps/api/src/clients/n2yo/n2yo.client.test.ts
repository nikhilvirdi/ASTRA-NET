...
describe('fetchN2yoPositions', () => {
  ...
  it('parses successful positions correctly', async () => {
    global.fetch = vi.fn(async () => new Response(JSON.stringify(positionsFixture), { status: 200 }));
...
describe('fetchN2yoVisualPasses', () => {
  ...
  it('parses successful passes correctly', async () => {
    global.fetch = vi.fn(async () => new Response(JSON.stringify(passesFixture), { status: 200 }));
    
    const params = { satId: 25544, observerLat: 40.7, observerLng: -74, observerAlt: 0, days: 2, minVisibility: 300 };
  ...
  it('returns empty array when passescount is 0', async () => {
    const noPassesFixture = { info: { satname: 'ISS', satid: 25544, transactionscount: 0, passescount: 0 } };
    global.fetch = vi.fn(async () => new Response(JSON.stringify(noPassesFixture), { status: 200 }));
