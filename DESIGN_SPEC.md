...
**Vertical rhythm below the horizon rule (resolved 2026-07-30 — see `DECISIONS.md`).** The band originally compressed the below-horizon glyph zone, the compass tick labels, and the below-horizon marker-label row into a fixed 50px budget between the rule and the fact plate. Past roughly -25° to -32° altitude (documented estimates disagreed; the real crossover, checked against the vendored font's own metrics, is -25.3°) a deep marker's glyph could touch a compass label — logged in `NOTES.md` rather than half-solved by shrinking the below-horizon range to an illegibly shallow band. Resolved by growing the budget: the compass-label offset, the below-horizon label baseline, and the fact plate (and everything anchored to it — the footer, the CTA) all shift downward by a fixed 14px, using headroom already unused at the card's bottom edge. The below-horizon glyph range itself (`HORIZON_MAX_DROP`) is unchanged — it was already a deliberate floor, not the source of the problem. Every gap below the fact plate keeps its original size, just displaced; nothing else in the composition changed. Verified by an exhaustive sweep (every 0.1° from 0 to -90°, the entire possible range, not spot checks) in `og-svg.test.ts`, using the real font metrics `font-metrics.ts` now exposes (`capHeight`/`descender`, read from each vendored face's own `OS/2` table) rather than an assumed fraction of font size.
